namespace niki {
    public class VideoGrid : Gtk.Grid {
        ...
        private VideoPresetList? video_preset_list;
        ...
        private const string [] CHROMA = {StringPot.Gamma, StringPot.Brightness, StringPot.Contrast, StringPot.Saturation, StringPot.Hue};
        ...
        public void init () {
            ...
            if (preset != null) {
                video_preset_list.select_preset (preset);
            }
            video_switch ();
            apply_changes = true;
        }
        ...
        public bool verify_preset_name (string preset_name) {
            ...
            foreach (var preset in video_preset_list.get_presets ()) {
        ...
        private void build_ui () {
            ...
            video_preset_list = new VideoPresetList ();
            video_preset_list.hexpand = true;

            side_list = new Gtk.Grid ();
            side_list.add (video_preset_list);
            ...
            size_group.add_widget (video_preset_list);
            ...
            video_preset_list.delete_preset_chosen.connect (remove_preset_clicked);
            video_preset_list.preset_selected.connect (preset_selected);
        ...
        private void load_presets () {
            foreach (var preset in VideoMix.get_default_presets ()) {
                ...
                video_preset_list.add_preset (preset);
            }
        ...
        private void notify_current_preset () {
            if (NikiApp.settingsVf.get_boolean ("videofilter-enabled")) {
                NikiApp.settings.set_string ("tooltip-videos", video_preset_list.get_selected_preset ().name);
            } else {
        ...
        private void on_default_preset_modified () {
            ...
            adding_preset = true;
            side_list.remove (video_preset_list);
        ...
        private void add_new_preset () {
            ...
            video_preset_list.add_preset (new_preset);
            side_list.add (video_preset_list);
            side_list.set_focus_child (video_preset_list);
        ...
        private string create_new_preset_name (bool from_current) {
            string current_preset_name = from_current ? video_preset_list.get_selected_preset ().name : "";
            string preset_name = "";
            bool is_valid = false;
            int i = 0;
        ...
        private void remove_preset_clicked () {
            video_preset_list.remove_current_preset ();
        }
    }
}
